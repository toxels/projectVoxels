...
void setPlanet(Planet planet, voxel* world)
{
	uint3 coords = planet.coords;
	int radius = planet.radius;
	uint3 centralDot = make_uint3(coords.x + radius, coords.y + radius, coords.z + radius);
	
	for(int i = 0; i<radius*2; i++)
		for(int j = 0; j<radius*2; j++)
			for (int k = 0; k<radius*2; k++)
			{
				uint3 currDot = make_uint3(coords.x + i, coords.y + j, coords.z + k);
				if(currDot.x > MAP_SIZE || currDot.y > MAP_SIZE || currDot.z > MAP_SIZE)
				{
					continue;
				}
				if(sqrt((centralDot.x - currDot.x) * (centralDot.x - currDot.x) +
							 (centralDot.y - currDot.y) * (centralDot.y - currDot.y) +
							 (centralDot.z - currDot.z) * (centralDot.z - currDot.z)) < radius)
				{
					world[currDot.x * MAP_SIZE * MAP_SIZE + currDot.y * MAP_SIZE + currDot.z].setActive();
					world[currDot.x * MAP_SIZE * MAP_SIZE + currDot.y * MAP_SIZE + currDot.z].color = planet.color;
...
void deletePlanet(Planet planet, voxel* world)
{
	uint3 coords = planet.coords;
	int radius = planet.radius;
	uint3 centralDot = make_uint3(coords.x + radius, coords.y + radius, coords.z + radius);
		...
		for (int j = 0; j < radius * 2; j++)
			for (int k = 0; k < radius * 2; k++)
			{
				...
				if (sqrt((centralDot.x - currDot.x) * (centralDot.x - currDot.x) +
					...
					(centralDot.z - currDot.z) * (centralDot.z - currDot.z)) < radius)
				{
					world[currDot.x * MAP_SIZE * MAP_SIZE + currDot.y * MAP_SIZE + currDot.z].setInactive();
				}
			}
...
int main() {
	...
	if (cudaMallocManaged(&world, MAP_SIZE * MAP_SIZE * MAP_SIZE * sizeof(world[0])))
		fprintf(stderr, "cuda malloc error: world");
		*/
	if (cudaMallocManaged(&world, MAP_SIZE * MAP_SIZE * MAP_SIZE * sizeof(voxel)))
		fprintf(stderr, "cuda malloc error: testVoxelModel");
	...
	int planetsCnt = 4;
	Planet planets[4];
	//planetCoords[0] = make_uint3(MAP_SIZE / 3, MAP_SIZE / 2, MAP_SIZE / 3);
	planets[0] = Planet(make_uint3(0, 0, 0), 1, 0, 7, MAP_SIZE / 5, make_uchar3(161, 37, 27));
	planets[1] = Planet(make_uint3(0, 0, 0), 1.5, 1.5, 5, MAP_SIZE / 8, make_uchar3(202,165,180));
	planets[2] = Planet(make_uint3(0, 0, 0), 0.5, 3.14, 10, MAP_SIZE / 3, make_uchar3(154,190,183));
	planets[3] = Planet(make_uint3(0, 0, 0), 0.7, 4.64, 8, MAP_SIZE / 6, make_uchar3(0,255,127));
	...
	while (window.isOpen()) {
		...
		//localLight.x = static_cast<int>((MAP_SIZE / 3) * cos(t)) + MAP_SIZE / 2;
		//localLight.z = static_cast<int>((MAP_SIZE / 3) * sin(t)) + MAP_SIZE / 2;
		//localLight.y = MAP_SIZE / 2;
		...
		for (int i = 0; i < planetsCnt; i++) {
			...
			deletePlanet(planets[i], world);
			planets[i].coords.x = static_cast<int>(planets[i].orbit * cos(t * planets[i].speed + planets[i].phase)) + MAP_SIZE / 2;
			planets[i].coords.z = static_cast<int>(planets[i].orbit * sin(t * planets[i].speed + planets[i].phase)) + MAP_SIZE / 2;
			planets[i].coords.y = MAP_SIZE / 2;
			setPlanet(planets[i], world);
		}
